...
var eq:UnaryFunction<any,UnaryFunction<any,boolean>> = function(key){
  return function(x){
    return x ===key
  }
}
var findParam:UnaryFunction<string,IO<Maybe<string[]>>> = function (key:string) {
  return _.map(_.compose(Maybe.of, _.filter<string[]>(_.compose(eq(key), _.head)), params), url) as IO<Maybe<string[]>>;
};
